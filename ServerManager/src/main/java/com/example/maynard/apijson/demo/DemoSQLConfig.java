...
package com.example.maynard.apijson.demo;
/*Copyright ©2016 TommyLemon(https://github.com/TommyLemon/APIJSON)
...
import static apijson.orm.AbstractVerifier.ACCESS_MAP;
...
import com.example.maynard.apijson.demo.model.*;
...
public class DemoSQLConfig extends APIJSONSQLConfig {
    ...
    static {
        ...
        TABLE_KEY_MAP.put(TUser.class.getSimpleName(), "t_user");
        TABLE_KEY_MAP.put(Privacy.class.getSimpleName(), "apijson_privacy");
        TABLE_KEY_MAP.put(Message.class.getSimpleName(), "message");
        ...
        ACCESS_MAP.put(Message.class.getSimpleName(),
                getAccessMap(Message.class.getAnnotation(MethodAccess.class)));


        ACCESS_MAP.put(Moment.class.getSimpleName(), getAccessMap(Moment.class.getAnnotation(MethodAccess.class)));
        ACCESS_MAP.put(TUser.class.getSimpleName(), getAccessMap(TUser.class.getAnnotation(MethodAccess.class)));
    ...
    public String getDBVersion() {
        if (isMySQL()) {
            return "5.1.47";  // "8.0.11";  // TODO 改成你自己的 MySQL 或 PostgreSQL 数据库版本号  // MYSQL 8 和 7 使用的 JDBC 配置不一样
        }
        if (isPostgreSQL()) {
    ...
    public String getDBUri() {
        if (isMySQL()) {
            ...
            // 以下是 MySQL 5.7 及以下
            return "jdbc:mysql://192.168.202.154:3303/apijson?useUnicode=true&characterEncoding=utf8&serverTimezone=GMT%2B8&useSSL=false"; // TODO 改成你自己的，TiDB 可以当成 MySQL 使用，默认端口为 4000
        }
        if (isPostgreSQL()) {
