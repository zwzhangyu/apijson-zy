...
export default {
  ...
  methods: {
    ...
    queryList() {
      ...
      this.$http.get('http://localhost:8080/api/server/paging', {params: params}).then(({body}) => {
    ...
    batchRemove() {
      ...
      this.$http.post("http://localhost:8080/api/server/batchRemove", {}, {params: params}).then(({body}) => {
        if (body.success === true) {
          this.queryList();
    ...
    deleteServer(id) {
      ...
      this.delete('http://localhost:8080/api/server/delete', params);
    },
    createServer(row) {
      console.log(row);
        ...
        if (valid) {
          if (params.applyTime > params.releaseTime) {
            ...
            if (this.messageForm.id) {
              ...
              this.save('http://localhost:8080/api/server/apply', params, 'messageVisible');
            } else {
              params.serverStatus = this.messageForm.serverStatus;
              this.save('http://localhost:8080/api/server/create', params, 'messageVisible');
            }
          }
        }
      })
    ...
    releaseServer(id) {
      this.$http.post('http://localhost:8080/api/server/release?id=' + id, {}, {}).then(({body}) => {
        if (body.success === true) {
          this.$message.success(body.message);
          this.queryList();
        } else {
